...
Raytracer::Sphere::Sphere(const Math::Point3D &center, double radius, Color color) :
    _center(center.x, center.y, center.z), _radius(radius), _color(color)
{
}
...
Raytracer::HitInfo Raytracer::Sphere::hits(Raytracer::Ray &ray)
{
    ...
    return HitInfo(true, coincide, _color);
}
...
const Raytracer::Math::Point3D Raytracer::Sphere::getCenter() const
{
    return _center;
}
